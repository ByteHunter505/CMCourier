"""Tests unitarios para :func:`cmcourier.services.lane_splitter.split` (036).

El `splitter` es una función pura. Los tests ejercitan:

* `batch` chico → fallback a un solo `lane` (no vale la pena el
  overhead de split).
* `batch` bimodal → partición correcta por ``threshold_bytes``.
* `batch`es degenerados todo-chico / todo-grande → fallback a un
  solo `lane` (un "`lane`" con cero items no aporta nada).
* Preservación del orden dentro de cada `lane`.
* Accesor ``size_of`` polimórfico (funciona con cualquier tipo de item).
"""
...
class TestPartitioning:
    ...
    def test_exact_threshold_lands_in_heavy(self) -> None:
        ...
        result = split(items, threshold_bytes=10 * _MB, min_batch=10, size_of=_size_of)
        assert result.is_single_lane is False
        # >= threshold → `heavy`. Los items de 10 MB van todos a `heavy`.
    ...
    def test_strictly_above_threshold_lands_in_heavy(self) -> None:
        ...
        items = [item_heavy, item_light] * 25  # 50 items, split 25/25
        result = split(items, threshold_bytes=10 * _MB, min_batch=10, size_of=_size_of)
...
class TestDegenerateFallback:
    def test_all_small_collapses_to_single_lane(self) -> None:
        ...
        result = split(items, threshold_bytes=10 * _MB, min_batch=10, size_of=_size_of)
        # Cada item < threshold → `lane` `heavy` vacío → single-lane.
        assert result.is_single_lane is True
        assert result.heavy == ()
        assert result.light == tuple(items)
    ...
    def test_all_large_collapses_to_single_lane(self) -> None:
        ...
        result = split(items, threshold_bytes=10 * _MB, min_batch=10, size_of=_size_of)
        # Cada item >= threshold → `lane` `light` vacío → single-lane.
        assert result.is_single_lane is True
        assert result.heavy == ()
        assert result.light == tuple(items)
